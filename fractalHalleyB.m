...
function fractalNewtonB(stepcount,xyrange)    
...
iterationcount = 1000;                                     % Setting max # of 
                                                           % iterations to
                                                           % find zero.
complexroots = roots([1 0 0 -sqrt(-1) 1 1 -2]);            % finds roots
polyDegree = length([1 0 0 -sqrt(-1) 1 1 -2])-1;
...
for u = x
    for v = y
        ...
        while ((err > 10^(-6)) & (iterate<iterationcount))   % Comparing distances between
            ...
            Zknew = Halley(p2(Zk), dp2(Zk), ddp2(Zk), Zk)
        ...
        [dist,rootIterations]= 
            min(abs(root-complexroots));                    % Matlab protocals to plot different colors for 
            colourcount(rootIterations) =
            colourcount(rootIterations) + 1;
            colourrecord(rootIterations, 
                        colourcount(rootIterations)) = z0;
    end
end
